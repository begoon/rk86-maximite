...
MMC_RESPONSE SendMMCCmd(BYTE cmd, DWORD address)
{
    ...
    // Commented out by Alexander Demin for Radio-86RK // 17.08.2012
    // while(CheckingSD);                      // wait if the card is being checked for removal // grg 8/4/11
    SD_CS_SET_LO;			// SD_CS = 0;                           //Select card
    ...
    switch(info->bStateVariable)
    {
        ...
        case ASYNC_READ_NEW_PACKET_READY:
            ...
            else
            {
                ...
                SD_CS_SET_HI; 				//SD_CS = 1;  //De-select media
                mSend8ClkCycles();  
...
BYTE MDD_SDSPI_AsyncWriteTasks(ASYNC_IO* info)
{
    ...
    switch(info->bStateVariable)
    {
        ...
        case ASYNC_WRITE_ABORT:
            ...
            SendMMCCmd(STOP_TRANSMISSION, 0x00000000);
            SD_CS_SET_HI; 				//SD_CS = 1;  //deselect media
...
MEDIA_INFORMATION *  MDD_SDSPI_MediaInitialize(void)
{
    ...
    SD_CS_SET_HI; 				//SD_CS = 1;               //Initialize Chip Select line (1 = card not selected)
    ...
    else
	{
        ...
        SD_CS_SET_HI; 				//SD_CS = 1;                              // deselect the devices
        Delayms(1);
        // Commented out by Alexander Demin for Radio-86RK // 17.08.2012
        // while(CheckingSD);                      // wait if the card is being checked for removal // grg 8/4/11
        SD_CS_SET_LO;			// SD_CS = 0;                              // select the device
