            ...
            case PS2STOP:
                if (d) {                       // PS2DAT == 1
                    if (code == 0xf0)
                        ...
                        rk86_keyboard(code, key_E0, key_up_code);
                        ...
                        // F12 to reset the Maximite hardware. If to hold the
                        // bootloader button it will restart in the bootloader
                        // mode.
                        if (code == 0x07) {
                            maximite_reset();
                        }
                        
                        if (code == 0x01) {  // F9 for I8080 soft reset (without zeroing memory).
                            i8080_pic32_soft_reset();
                        }

                        if (code == 0x09) {  // F10 for I8080 hard reset (with zeroing memory).
                            i8080_pic32_hard_reset();
                        }
